...
public class TasksAdapter extends RecyclerView.Adapter<TaskViewHolder> {
    ...
    public void onBindViewHolder(@NonNull TaskViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.rejectATask.setOnClickListener(v -> {
            ...
            builder.setPositiveButton("Да",
                ...
                databaseTasks = new DatabaseTasks(holder.itemView.getContext());
                databaseTasks.openDatabase();
                databaseTasks.deleteTask(databaseTasks.getTaskByTaskId(tasks.get(position).getTaskId()).getId());
                databaseTasks.close();
                removeItem(holder);
            });
    ...
    private void setTaskStatus(TaskViewHolder holder, int newTaskStatus, int position) {
        ...
        databaseTasks = new DatabaseTasks(holder.itemView.getContext());
        databaseTasks.openDatabase();
        databaseTasks.updateStatus(databaseTasks.getTaskByTaskId(tasks.get(position).getTaskId()).getId(), newTaskStatus);
        databaseTasks.close();
    ...
    private void setVisibilityComponents(TaskViewHolder holder, int position) {
        holder.taskText.setText(tasks.get(position).getUserFromId());
    ...
    private void loadTaskInfo(TaskViewHolder holder, int position) {
        holder.taskText.setText(tasks.get(position).getTaskText());// + " | " + tasks.get(position).getTaskId()
        ...
        holder.termDateTime.setText(getItem(position).getTermDateTime());
...
//                    holder.user.setText(tasks.get(position).getUserFromId());
//                } else {
//                    holder.user.setText(tasks.get(position).getUserToId());
//                }
    }
    ...
    private int getTaskColor(int position) {
        int taskColor = Color.WHITE;
        int[] taskColors = new int[]{Color.argb(50, 100, 120, 70), Color.argb(100, 50, 251, 155), Color.argb(100, 101, 150, 0), Color.argb(100, 42, 99, 255)};
        if (getItem(position).getDegreeOfImportance()>-1 && getItem(position).getDegreeOfImportance()<4) {
            taskColor = taskColors[getItem(position).getDegreeOfImportance()];
        }
        return taskColor;
    }
}
