...
public class UsersAdapter extends RecyclerView.Adapter<UserViewHolder> {
    ...
    public void onBindViewHolder(@NonNull UserViewHolder holder, @SuppressLint("RecyclerView") int position) {
        databaseFriends = new DatabaseFriends(holder.itemView.getContext());
        ...
        if (!users.get(position).getUserImage().equals("null")) {
            Glide.with(holder.itemView.getContext()).load(users.get(position).getUserImage()).into(holder.profileImage);
        } else {
        ...
        holder.addNewFriend.setOnClickListener(v -> {
            ...
            removeItem(holder);
            makeToast(holder.itemView.getContext(), "Заявка в друзья отправлена");
        });
        holder.deleteFriend.setOnClickListener(v -> {
            ...
            builder.setPositiveButton("Да, уверен",
                    (dialog, which) -> {
                        ...
                        databaseFriends.deleteFriend(databaseFriends.getFriendByUserId(users.get(position).getUserId()).getId());
        ...
        holder.acceptRequestFriend.setOnClickListener(view -> {
            ...
            databaseFriends.openDatabase();
            databaseFriends.insertFriend(new User(users.get(position).getUsername(), users.get(position).getUserId()), holder.itemView.getContext());
            databaseFriends.close();
            removeItem(holder);
        });
